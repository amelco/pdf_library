...
implicit none
  ...
  do while (slc .ne. "Q" .and. slc .ne. "q")
	...
	! -- To improve the code, a search for the right file with the given id must be made.
        ...
        cmd = reader//' '//trim(adjustl(path))//'`cat '//datafile//' | grep -i '''//trim(adjustl(slc))//';'' | awk ''BEGIN{FS=";"}{print $2}'' | head -1`'//"& > /dev/null 2>&1"
	call system(cmd)
	slc = "I"
    end select
  end do
...
implicit none
  ...
  print*
  print*, "Put an '*' at the end of the string to show all results that contains the string that you want. Ex.: solute* will show either solute, absolute, absolutely..."
  print*
  print*, "Type the search string:" 
  read(*,'(A)') str
  call system("clear")
  ...
  if (index(str,'*') > 0) then
    ...
    cmd = "find "//trim(adjustl(path))//" -name '*.pdf' -exec sh -c 'pdftotext ""{}"" - | grep -i --with-filename --label=""{}"" --color=always """//trim(adjustl(str))//"""' \; | less -r 2>/dev/null"
  else
    cmd = "find "//trim(adjustl(path))//" -name '*.pdf' -exec sh -c 'pdftotext ""{}"" - | grep -i -w --with-filename --label=""{}"" --color=always """//trim(adjustl(str))//"""' \; | less -r 2>/dev/null"
  endif
  print*, cmd
  call system(cmd)
  print*
...
!  cmd = 'sed ''1 c\ `ls -l *.dat | awk ''BEGIN{i=1} {print "  ["i"]",$9; i++}'' | awk ''NR=='//slc//' {print $2}''` settings.in'
  cmd = "sed -i '1s/.*/'""`ls -l *.dat | awk 'BEGIN{i=1} {print ""[""i""]"",$9; i++}' | awk 'NR=="//slc//" {print ""\042""$2""\042""}'`""'/' settings.in" 
  call system(cmd)
  call read_settings()
  print*
